...
class JazzImprovisationEnv(gym.Env):
    ...
    def _calculate_reward(self, action):
        ...
        # 1. HARMONY
        if is_note:
            if (action % 12) in chord_notes:
                reward += 1.0
            else:
                reward -= 0.6  # Λιγότερο αυστηρό για να επιτρέψει περάσματα

        # 2. MELODIC FLOW & SCALE RUNS (Humanizer)
        if is_note and prev1 < 36:
            interval = abs(action - prev1)
            # Step-wise motion (1-2 semitones) = Scale Run -> BIG BONUS
            if 1 <= interval <= 2:
                reward += 0.8
                # Small jump (3-5 semitones) -> Small Bonus
            elif 3 <= interval <= 5:
                reward += 0.4
            # Large jump -> Penalty (unless resolved later, hard to check)
            elif interval > 9:
                reward -= 0.3

        # 3. PHRASE REPETITION (Anti-Loop)
        last_3 = self.history[-3:]
        prev_3 = self.history[-6:-3]
        if last_3 == prev_3: reward -= 4.0
        last_4 = self.history[-4:]
        prev_4 = self.history[-8:-4]
        if last_4 == prev_4: reward -= 4.0
        ...
        # 5. REPETITION
        if is_note and action == prev1: reward -= 2.0

        # 6. DYNAMIC HOLD
        if is_hold:
            last_played = next((a for a in reversed(self.history[:-1]) if a < 36), -1)
            if last_played != -1 and (last_played % 12) in chord_notes:
